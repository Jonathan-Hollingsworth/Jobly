  ...
  router.get("/", async function (req, res, next) {
    try {
      ...
      if (minSalary) {
        if (title) {
          if (hasEquity) {
            const jobs = await Job.findByAll(title, minSalary);
      ...
      if (title) {
        if (hasEquity) {
          const jobs = Job.findByTitleAndEquity(title);
          return res.json({ jobs });
        }
        const jobs = await Job.findByTitle(title);
        return res.json({ jobs });
      }
      if (hasEquity) {
        const jobs = await Job.findByEquity();
        return res.json({ jobs });
      }
